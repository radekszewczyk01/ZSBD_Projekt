...
-- Tabela łącząca Autora z jego dziedzinami specjalizacji
CREATE TABLE IF NOT EXISTS Autor_Dyscyplina (
...
CREATE TABLE IF NOT EXISTS RundaRecenzyjna (
    ...
    -- DODANA KOLUMNA
    id_redaktora_prowadzacego INT, -- Klucz obcy do tabeli Autor
    ...
    -- DODANY KLUCZ OBCY
    FOREIGN KEY (id_redaktora_prowadzacego) REFERENCES Autor(id_autora)
        ON UPDATE CASCADE
        ON DELETE RESTRICT -- Używamy RESTRICT, aby przypadkowe usunięcie autora nie usunęło rundy
);
